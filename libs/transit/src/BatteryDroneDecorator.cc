...
void BatteryDroneDecorator::Update(double dt, std::vector<IEntity*> scheduler,
std::vector<IEntity*> stations) {
  if (drone->GetAvailability()) {
    GetNearestEntity(scheduler, stations);
  }
  if (onRechargeMission) {
    if (drone->GetPosition().Distance(nearestRechargeStation->GetPosition())
    < 50.0) {
    ...
    if (batteryLevel >= 95.0) {
      ...
      drone->SetToTargetPosStrategy(new BeelineStrategy(drone->GetPosition(),
      targetPos));
...
void BatteryDroneDecorator::GetNearestEntity(std::vector<IEntity*> scheduler,
std::vector<IEntity*> stations) {
  ...
  std::cout << "Distance from robot to destination: " << path2Distance
  << std::endl;
  ...
  if (!hasEnoughBatteryForDist(tripDistance)) {
    std::cout << std::endl << "Not enough battery for trip!" <<
    std::endl << std::endl;
...
void BatteryDroneDecorator::GetNearestRechargeStation(std::vector<IEntity*>
stations) {
